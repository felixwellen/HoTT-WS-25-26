...
\rednote{TODO: Hier sollte es eigentlich (wie in der VL) eine Erklärung geben, dass $f(x)$ im folgenden nichts mit $b(x)$ gemein hat. Besser wäre es, hier erstmal etwas wie $\mathrm{app}(f,x)$ einzuführen, um die Hörer/Leser etwas weniger zu verwirren.}
\begin{center}
...
In Worten sagt der Eliminator in etwa, dass es genug ist eine Konstruktion, die eine Gleichheit verwendet, für die reflexive Gleichheit ``$\refl_x:x=x$'' anzugeben.
...
\item Für alle $ p:x=y$ haben wir $ p\kon p^{-1}=\refl_x$ und $p^{-1}\kon p=\refl_y$.
...
Wie schon bei anderen Typen können wir auch für die abhängige Summe zeigen, dass alle Elemente Konstruktorwerte sind -- also Paare.
...
\begin{definition}
  Ein Typ $A$ heißt \begriff{kontrahierbar}\index{$\isContr$}, wenn der folgende Typ bewohnt ist:
\[
  \mathrm{\isContr}(A)\colonequiv (x:A)\times ((y:A)\to y=x)
\]
...
\begin{beweis}[Wie in der Vorlesung]
...
\item Alle Typformer erhalten Universenlevel bzw. hat ein geformter Typ als Universenlevel mindestens das Maximum der Universenlevel der eingehenden Typen.
\item Die Motive von Eliminatoren haben beliebiges Universenlevel.
\end{itemize}
\end{regeln}
