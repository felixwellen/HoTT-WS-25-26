...
\begin{beweis}
  \begin{enumerate}[(a)]
  \item   Es ist $(u,c)\mapsto (\pi_1(u),(\pi_2(u),\transp_C(p_u)(c)))$ eine Quasi-Inverse, wobei $p:(u:(x:A)\times B(x))\to u=(\pi_1(u),\pi_2(u))$ durch $p(x,b)\colonequiv \refl$ gegeben ist.
  Beide Homotopien sind direkt durch Induktion über das Argument und anwenden von $(\_,\_)_=$ gegeben.
...
\begin{lemma}
  Sei $B:A\to\mU$.
  \begin{enumerate}[(a)]
  \item Ist jedes $B(x)$ kontrahierbar, so ist $\pi_1:\sum_{x:A}B(x)\simeq A$ eine Äquivalenz.
  \item Ist $A$ kontrahierbar mit Zentrum $a:A$ und Kontraktion $k:(x:A)\to x=a$ die Abbildung
    $(x,b)\mapsto \transp_B(k_x)(b) : \sum_{x:A}B(x)\simeq B(a)$ eine Äquivalenz.
  \end{enumerate}
\end{lemma}
...
\begin{lemma}
  ...
  Dann gibt es eine kanonische Äquivalenz:
  \[
