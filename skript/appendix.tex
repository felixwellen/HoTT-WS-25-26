...
\begin{center}
  ...
  \end{tabular}
\end{center}
Statt $(x,p):(x:A)\times B(x)$ schreiben wir auch einfach $x:(x:A)\times B(x)$,
wenn $B(y)$ für jedes $y:A$ eine Aussage ist. Wir schreiben auch einfach nur $x$, wenn wir $(x,p):(x:A)\times B(x)$ verwenden.
Insbesondere schreiben wir zum Beispiel $f$ für eine Äquivalenz $(f,p):A\simeq B$.

Für $\sum_{x:\sum_{y:A}C(y)}B(x)$, 
schreiben wir auch $\sum_{(y,c):\sum_{y:A}C(y)}B((y,c))$.
...
\item Induktive Typformer haben höhere Priorität als $\to$, $\prod$ und $\sum$.
\item $\to$ hat höhere Priorität als $\prod$ und $\sum$.
\item $\times$ hat höhere Priorität als $\to$.
\item $\neg$ hat höhere Priorität als $\times$.
